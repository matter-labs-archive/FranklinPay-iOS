...
class LocalDatabase: ILocalDatabase {
    ...
    public func getAllTransactions(forWallet wallet: KeyWalletModel, andNetwork networkID: Int64) -> [ETHTransactionModel] {
        do {
            guard let result = try mainContext.fetch(self.fetchWalletRequest(withAddress: wallet.address)).first else {
                return []
            }
        ...
        container.performBackgroundTask { (context) in
            do {
                guard let wallet = try context.fetch(
                    self.fetchWalletRequest(withAddress: forWallet.address)
                    ).first else {
    ...
    public func getAllTokens(for wallet: KeyWalletModel, forNetwork: Int64) -> [ERC20TokenModel] {
        do {
            guard let wallet = try mainContext.fetch(self.fetchWalletRequest(withAddress: wallet.address)).first else {
                return []
            }
